...
- New `defineSchema(...)` + `Model({ schema })` overload for schema-driven prop inference. Pass a typed schema (`{ tableName, columns: { col: { type, null?, primary?, ... }, ... } }`) and TypeScript derives the prop shape from the column map at the call site — `{ id: 'integer', primary: true } → number`, `{ email: 'string' } → string`, `{ archivedAt: 'timestamp', null: true } → Date | null`, etc. — so `Model({ schema })` no longer needs the explicit `init: (props: ...) => props` callback that pinned the row shape today. Column kinds map to TS types as follows: `'string' | 'text' → string`, `'integer' | 'bigint' | 'float' | 'decimal' → number`, `'boolean' → boolean`, `'date' | 'datetime' | 'timestamp' → Date`, `'json' → unknown`. Primary columns drive the keys map (string primaries → `KeyType.uuid`; numeric primaries → `KeyType.number`); `tableName` falls through from the schema; `init` defaults to identity; all three remain overridable. The legacy `Model({ tableName, init, ... })` form is unchanged — both shapes coexist via overloads. The schema also produces a runtime `TableDefinition` (the same shape `defineTable(...)` returns) so tooling that reads `TableDefinition` (schema snapshots, GraphQL / OpenAPI generators, admin UIs) can consume the same declaration the Model factory uses.
