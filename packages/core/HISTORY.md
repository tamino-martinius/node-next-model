...
- Native UPSERT capability via a new optional `Connector.upsert(spec)` method. When the connector exposes it (the bundled `MemoryConnector` and `KnexConnector` both do — pg/sqlite/mysql/mariadb), `Model.upsert(...)` and `Model.upsertAll(...)` route through a single atomic `INSERT … ON CONFLICT … DO UPDATE` (or `ON DUPLICATE KEY UPDATE` on MySQL) instead of the old SELECT + INSERT/UPDATE dance — the documented "not atomic" caveat is gone for capable connectors, and bulk upserts collapse from N×2 round-trips to one (sqlite is internally chunked at 200 rows/statement to stay under `SQLITE_LIMIT_COMPOUND_SELECT`, all chunks share one transaction). New options `updateColumns: string[]` (whitelist columns that are overwritten on conflict) and `ignoreOnly: true` (`DO NOTHING` — keep the existing row untouched and return it) layer on top of the existing `onConflict` option. Connectors that don't define `upsert` (no breaking change for third-party connectors) keep the existing fallback path unchanged. Mirroring Rails' `upsert` / `upsert_all`, the native path skips per-row lifecycle callbacks and validators — use `Model.create` / `record.update` (or `Model.transaction(...)`) when those must run.
