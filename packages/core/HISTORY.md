...
- Cleanup pass: drop every legacy / backward-compat shim that landed during the migration. `Model.runQueryRecords` is gone — its body (afterFind callbacks, STI dispatch, includes attach/preload, `queryWithJoins` fast path, soft-delete / STI implicit filters, `pendingJoins` resolution) is now inlined in `CollectionQuery.materialize`; helper functions `applyIncludes` / `attachIncludesPayload` moved to `query/includes.ts` so they're reusable from the builder layer. `CollectionQuery.toScopedSubclass` is gone along with the 16 methods that used it (`pluckUnique`, `ids`, `countBy`, `exists`, `groupBy`, `updateAll`, `deleteAll`, `destroyAll`, `increment`, `decrement`, `paginate`, `paginateCursor`, `inBatchesOf`, `findEach`, `select`, multi-column `pluck`) — each is now a first-class builder method that calls connector primitives directly via a shared `resolvedScope()` helper. `ScalarQuery.viaLegacy` is gone; `ScalarQuery` and `ColumnQuery` resolve `pendingJoins` to flat `$in` / `$notIn` filters before calling `queryScoped`, so the connector path sees a clean spec; SQL connectors no longer fall back to `queryWithJoins` on pendingJoins+aggregate combos and throw if pendingJoins arrive at queryScoped. `Model.modelScope` / `Model.modelScopeBase` are gone — the same logic lives in `query/scope.ts` (`builderScopeBase`, `resolvePendingJoinsToScope`, `resolveParentScopesToFilter`) where the builder pipeline owns it. Static method types tightened: `Model.findBy(...)` returns `InstanceQuery<InstanceType<M> | undefined>` directly, `Model.count()` returns `ScalarQuery<number>`, `Model.all()` returns `CollectionQuery<InstanceType<M>[]>`, etc. — every `as unknown as Promise<…>` cast is gone, so `User.findBy({email}).pluck('name')` typechecks without intermediate awaits. Manual `instance.belongsTo(...)` / `instance.hasOne(...)` accessors return `InstanceQuery` rather than wrapping in `Promise.resolve(...)` — symmetric with auto-installed accessors and the `hasMany` shape; awaiting still works (InstanceQuery is PromiseLike). Cursor encode/decode helpers extracted to `query/cursor.ts`. `mergeFiltersForLegacy` renamed to `andMergeFilters` now that it isn't part of a legacy path.
...
- **`scopes: { name: (self) => self.filterBy(...) }` callbacks → declarative literals.** Named `scopes` are now `Filter<any>` literals — convert simple cases to `scopes: { name: { ...filter } }`. For parameterized / multi-step cases, declare a static method on your subclass (`static admins() { return this.filterBy({role: 'admin'}); }`); it composes with every chainable. The `ScopeFn` type is removed; `ScopeMap` is now `Dict<Filter<any>>`. Enum value scopes (`Post.draft()`) auto-generate the same shape.
