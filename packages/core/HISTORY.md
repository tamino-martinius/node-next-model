...
- New optional `Connector.atomicUpdate(spec)` capability. `spec` is `{ tableName, filter, deltas: [{ column, by }], set? }` and applies every `column = column + by` in a single round-trip; the optional `set` lands absolute writes (e.g. `updatedAt = now`) alongside the deltas. Returns the affected row count. Implemented natively on every shipped connector — `postgres-connector` / `mysql-connector` / `mariadb-connector` / `sqlite-connector` / `aurora-data-api-connector` / `knex-connector` use SQL `UPDATE col = COALESCE(col, 0) + ?`; `mongodb-connector` uses `$inc`; `redis-connector` / `valkey-connector` use `HINCRBY` / `HINCRBYFLOAT` per-row inside a `MULTI`; `MemoryConnector` / `local-storage-connector` walk in-process. Wired into `record.increment(col, by)` / `record.decrement(col, by)` (skips validation + full save callbacks à la Rails `update_columns`; still fires `afterUpdate` / `afterUpdateCommit`; bumps `updatedAt` automatically), the new chainable class-level `Model.where(...).increment(col, by)` / `Model.where(...).decrement(col, by)` (returns affected count), and the `counterCaches` afterCreate / afterDelete / afterUpdate hooks (one round-trip, no parent SELECT). Race-free under concurrency: 1 000 concurrent `Comment.create({ postId: 1 })` calls land the right counter value.
