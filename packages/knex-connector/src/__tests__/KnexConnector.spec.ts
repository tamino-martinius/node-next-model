...
describe('KnexConnector', () => {
  describe('#query', () => {
    ...
    describe('filter operators', () => {
      ...
      it('$gt / $gte / $lt / $lte', async () => {
        expect(ids((await connector.query({ tableName, filter: { $gt: { age: 20 } } })) as any))
          .toEqual([bob.id, carol.id]);
        expect(ids((await connector.query({ tableName, filter: { $gte: { age: 21 } } })) as any))
          .toEqual([bob.id, carol.id]);
        expect(ids((await connector.query({ tableName, filter: { $lt: { age: 21 } } })) as any))
          .toEqual([alice.id]);
        expect(ids((await connector.query({ tableName, filter: { $lte: { age: 18 } } })) as any))
          .toEqual([alice.id]);
      });
      ...
      it('rejects filter with zero keys in $in with FilterError', async () => {
        await expect(
          connector.query({ tableName, filter: { $in: {} } }),
        ).rejects.toBeInstanceOf(FilterError);
      });
    });
  });
  ...
  describe('#execute', () => {
    ...
    it('runs raw SQL with named bindings', async () => {
      const rows = await connector.execute('SELECT * FROM users WHERE age = :age', { age: 18 } as any);
      expect(rows).toHaveLength(1);
      expect(rows[0].name).toBe('alice');
    });
  });
