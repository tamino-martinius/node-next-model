                        ...
                        filterSpecGroups[groupName].forEach(filterSpec => {
                            context(`with filter '${JSON.stringify(filterSpec.filter)}'`, {
                                ...
                                tests() {
                                    const results = filterSpec.results;
                                    if (Array.isArray(results)) {
                                        const itUpdatesMatchingItems = (results) => {
                                            it('promises to return updated records', async () => {
                                                await expect(subject()).resolves.toEqual(results.map(id => ({ id, ...attrs })));
                                            });
                                            if (results.length === 0) {
                                                ...
                                                it('changes matching items in storage', async () => {
                                                    const storageBeforeUpdate = clone(items());
                                                    const changedStorage = items().map(item => results.includes(item.id) ? { ...item, ...attrs } : item);
                                                    await subject();
                                                    const storageAfterUpdate = items();
                                                    expect(storageBeforeUpdate).not.toEqual(storageAfterUpdate);
                        ...
                        filterSpecGroups[groupName].forEach(filterSpec => {
                            context(`with filter '${JSON.stringify(filterSpec.filter)}'`, {
                                ...
                                tests() {
                                    const results = filterSpec.results;
                                    if (Array.isArray(results)) {
                                        const itDeletesMatchingItems = (results) => {
                ...
                context(description, {
                    ...
                    tests() {
                        ...
                        context('with multiple items to insert', {
                            definitions: () => (itemsToInsert = [{ foo: 'bar' }, { foo: null }, { foo: undefined }]),
                            reset: () => (itemsToInsert = []),
                            tests() {
...
//# sourceMappingURL=MemoryConnector.spec.mjs.map