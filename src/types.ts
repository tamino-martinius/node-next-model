...
> {
  ...
  readonly attributes: PersistentProps &
    Partial<PersistentProps> &
...
> {
  ...
  createScoped(
    props: Partial<CreateProps>,
  ): Promise<ModelInstance<CreateProps, PersistentProps, Keys>>;
  ...
  select(
    ...keys: [keyof Keys | keyof PersistentProps][]
  ): Promise<
    Partial<
      PersistentProps & { [K in keyof Keys]: Keys[K] extends KeyType.uuid ? string : number }
    >[]
